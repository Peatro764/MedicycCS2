...
{            
    ...
    QObject::connect(&socket_, static_cast<void (QTcpSocket::*)(QAbstractSocket::SocketError)>(&QAbstractSocket::errorOccurred),
...
void SocketClient::Connect() {
    if (socket_.state() == QAbstractSocket::UnconnectedState) {
        QString msg(ip_ + ":" + QString::number(port_));
...
void SocketClient::Flush() {
    qDebug() << QDateTime::currentDateTimeUtc().toString() << " SocketClient::Flush";
    socket_.flush();
...
void SocketClient::HandleConnectionTimeout() {
    ...
    socket_.abort();
    emit PortError("Connection timeout");
}
