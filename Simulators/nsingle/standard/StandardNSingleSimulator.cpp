...
#include "ui_StandardNSingleSimulator.h"
...
StandardNSingleSimulator::StandardNSingleSimulator(hw_nsingle::VirtualNSingle *nsingle, QWidget *parent) :
    QWidget(parent),
    ui_(new Ui::StandardNSingleSimulator),
    nsingle_(nsingle)
{
    ui_->setupUi(this);
...
StandardNSingleSimulator::~StandardNSingleSimulator()
{

}

void StandardNSingleSimulator::ConnectGuiSignals() {
    qDebug() << "StandardNSingleSimulator::ConnectGuiSignals";
    ...
    QObject::connect(ui_->pushButton_ch1_value_inc, &QPushButton::clicked, this, &StandardNSingleSimulator::IncrementChannel1Value);
    QObject::connect(ui_->pushButton_ch1_value_dec, &QPushButton::clicked, this, &StandardNSingleSimulator::DecrementChannel1Value);
    QObject::connect(ui_->lineEdit_ch1_setpoint, &QLineEdit::returnPressed, this, &StandardNSingleSimulator::Channel1SetUpdated);
    QObject::connect(ui_->lineEdit_ch1_value, &QLineEdit::returnPressed, this, &StandardNSingleSimulator::Channel1ValueUpdated);
    QObject::connect(ui_->lineEdit_ch1_value_bits, &QLineEdit::returnPressed, this, &StandardNSingleSimulator::Channel1ValueBitSetUpdated);
    QObject::connect(ui_->lineEdit_address_value, &QLineEdit::returnPressed, this, &StandardNSingleSimulator::AddressValueUpdated);
    QObject::connect(ui_->lineEdit_address_value_bits, &QLineEdit::returnPressed, this, &StandardNSingleSimulator::AddressValueBitSetUpdated);
    ...
    QObject::connect(nsingle_, &hw_nsingle::VirtualNSingle::SIGNAL_CommandReadState, this, [&]() {
        ...
        if (ui_->okRadioButton->isChecked()) QTimer::singleShot(80, this, [&]() { emit nsingle_->SIGNAL_Ok(); });
        }
    });
    QObject::connect(nsingle_, &hw_nsingle::VirtualNSingle::SIGNAL_CommandPing, this, [&]() {
        qDebug() << "Ping";
        emit nsingle_->SIGNAL_IOLoad(ui_->verticalSlider_ioload->value());
        ...
        if (ui_->pushButton_readstatus_active->isChecked()) {
            if (ui_->onRadioButton->isChecked()) QTimer::singleShot(80, this, [&]() { emit nsingle_->SIGNAL_On(); qDebug() << "EMit ON"; });
            if (ui_->offRadioButton->isChecked()) QTimer::singleShot(80, this, [&]() { emit nsingle_->SIGNAL_Off(); qDebug() << "EMIT OFF "; });
...
QString StandardNSingleSimulator::SignToString(bool sign) const {
    if (sign) {
        return QString("+");
    } else {
        return QString("-");
...
bool StandardNSingleSimulator::Ch1SetPointSign() const {
    return (ui_->label_ch1_setpoint_sign->text() == QString("+"));
}

bool StandardNSingleSimulator::Ch1ValueSign() const {
    return (ui_->pushButton_ch1_value_sign->text() == QString("+"));
}

bool StandardNSingleSimulator::Ch2ValueSign() const {
    return (ui_->label_ch2_value_sign->text() == QString("+"));
}

void StandardNSingleSimulator::Channel1SetUpdated() {
    try {
            hw_nsingle::Measurement m(nsingle_->config().GetChannel1SetPointConfig());
...
void StandardNSingleSimulator::Channel1ValueUpdated() {
    try {
            hw_nsingle::Measurement m(nsingle_->config().GetChannel1ValueConfig());
...
void StandardNSingleSimulator::Channel1ValueBitSetUpdated() {
    try {
            hw_nsingle::Measurement m(nsingle_->config().GetChannel1ValueConfig());
            std::bitset<15> bits(ui_->lineEdit_ch1_value_bits->text().toStdString());
            m.SetData(static_cast<int>(bits.to_ulong()), Ch1ValueSign());
...
void StandardNSingleSimulator::IncrementChannel1Value() {
    try {
            hw_nsingle::Measurement m(nsingle_->config().GetChannel1ValueConfig());
            std::bitset<15> bits(ui_->lineEdit_ch1_value_bits->text().toStdString());
            m.SetData(static_cast<int>(bits.to_ulong()), Ch1ValueSign());
...
void StandardNSingleSimulator::DecrementChannel1Value() {
    try {
            hw_nsingle::Measurement m(nsingle_->config().GetChannel1ValueConfig());
            std::bitset<15> bits(ui_->lineEdit_ch1_value_bits->text().toStdString());
            m.SetData(static_cast<int>(bits.to_ulong()), Ch1ValueSign());
...
void StandardNSingleSimulator::AddressValueUpdated() {
    try {
...
void StandardNSingleSimulator::AddressValueBitSetUpdated() {
    try {
...
void StandardNSingleSimulator::DisplayAddressValue(const hw_nsingle::AddressRegister& address) {
...
void StandardNSingleSimulator::DisplayAddressSetPoint(const hw_nsingle::AddressRegister& address) {
...
void StandardNSingleSimulator::DisplayChannel1Value(const hw_nsingle::Measurement& m) {
...
void StandardNSingleSimulator::DisplayChannel1SetPoint(const hw_nsingle::Measurement& m) {
    const std::bitset<15> bits(m.RawValue());
...
void StandardNSingleSimulator::DisplayChannel2Value(const hw_nsingle::Measurement& m) {
    const std::bitset<15> bits(m.RawValue());
