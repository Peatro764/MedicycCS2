...
int main(int argc, char *argv[])
{
    ...
    if (QCoreApplication::arguments().size() != 2) {
        qWarning() << "Wrong number of arguments given. Format: ./InjectionNSingleGroup 3000";
        return -1;
    }
    const QString sub_system = "INJECTION";
    bool ok(false);
    int iteration_time = QCoreApplication::arguments().at(1).toInt(&ok);
    if (!ok) {
    ...
    medicyc::cyclotroncontrolsystem::middlelayer::nsinglegroup::injection::InjectionNSingleGroup nsingle_group(iteration_time);
